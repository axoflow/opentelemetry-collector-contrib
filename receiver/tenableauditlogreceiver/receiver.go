...
type checkpoint struct {
	...
	// SeenIDs holds the ids of the events sharing LastEventTime. The API filter granularity is
	// coarser than the event timestamps, so overlapping events must be deduplicated by id.
...
func (r *tenableReceiver) request(ctx context.Context, since time.Time, limit int, nextToken string) (*auditLogResponse, error) {
	query := url.Values{}
	query.Set("f", "date.gt:"+since.UTC().Format("2006-01-02T15:04:05Z"))
