...
// The window only counts as consumed once the checkpoint is persisted; advancing lastEnd first
// would let a failed Set skip a window that no restart could recover.
func (r *idiraReceiver) setCheckpoint(ctx context.Context, to time.Time) error {
	if err := r.storage.Set(ctx, checkpointKey, []byte(to.Format(time.RFC3339))); err != nil {
