...
func (f *fakeAPI) server(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/token", func(w http.ResponseWriter, r *http.Request) {
		// The documented client credentials flow: the client ID and secret go in a Basic
		// Authorization header, the grant type and scope in the request body.
		id, secret, ok := r.BasicAuth()
		assert.True(t, ok)
		assert.Equal(t, "siem-integration", id)
		assert.Equal(t, "a-secret", secret)
		assert.NoError(t, r.ParseForm())
		assert.Equal(t, "client_credentials", r.PostForm.Get("grant_type"))
		assert.Equal(t, "isp.audit.events:read", r.PostForm.Get("scope"))
...
// TestPollFailedCheckpointRequeriesWindow covers a failing storage extension: the window must not
// be treated as consumed, so the next poll still covers it instead of skipping past it.
func TestPollFailedCheckpointRequeriesWindow(t *testing.T) {
	api := newFakeAPI()
	srv := api.server(t)
	cfg := testConfig(srv.URL)
	advance := fakeClock(t)

	failing := &fakeStorage{data: map[string][]byte{}, setErr: errors.New("no space left on device")}
	r := newReceiver(cfg, receivertest.NewNopSettings(metadata.Type), new(consumertest.LogsSink))
	r.client = &client{httpClient: srv.Client(), endpoint: cfg.Endpoint, apiKey: string(cfg.APIKey)}
	r.storage = failing

	require.ErrorContains(t, r.poll(t.Context()), "no space left on device")
	assert.True(t, r.lastEnd.IsZero(), "an unpersisted window must not count as consumed")
	...
	require.Len(t, api.queries, 2)
	firstTo, err := time.Parse(dateLayout, api.queries[0].Query.FilterModel.Date.DateTo)
	require.NoError(t, err)
	secondFrom, err := time.Parse(dateLayout, api.queries[1].Query.FilterModel.Date.DateFrom)
	require.NoError(t, err)
	assert.False(t, secondFrom.After(firstTo), "the second window must not start past the first one's end")
}
