...
func (p *cloudwatchCheckpointPersister) GetCheckpoint(
	...
	if err != nil || data == nil || len(data) == 0 {
		return p.validateCheckpoint(logGroupName, data, err), nil
	}
...
// validateCheckpoint deals with various error cases when retrieving a checkpoint
// and returns the appropriate timestamp to start from
func (p *cloudwatchCheckpointPersister) validateCheckpoint(
	logGroupName string, data []byte, err error,
) string {
	switch {
	case err != nil:
		p.logger.Warn("Error retrieving checkpoint, starting from the beginning",
			zap.String("logGroup", logGroupName),
			zap.Error(err))
	case data == nil:
	...
	return newCheckpointTimeFromStartOfStream()
}
